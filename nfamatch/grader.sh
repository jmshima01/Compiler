...
basiciotallydir=`grader_mktemp -d basicio`
tally_missing_datafile "${basiciotallydir}/missingdata" "${comploc}/${PROGRAM}" MISSINGDATAFILE /dev/null 

tally_empty_datafile "${basiciotallydir}/emptydata" "${comploc}/${PROGRAM}" EMPTYDATAFILE /dev/null 
...
show_io_tallies()
{
	...
	local descr="${@}" 
	# tallies
	local total=`ls -1 "${tally}"*.[gb]* 2>/dev/null|wc -l | tr -d '[:space:]'`
	local good=`ls -1 "${tally}"*.good 2>/dev/null|wc -l | tr -d '[:space:]'`

	grader_msg_ << EOT 
${descr}: ${good} out of ${total} correct runtime outcome.
EOT
}
...
#
	grader_msg_ << EOT 
${descr}: ${goodtt} out of ${totaltt} correct transition tables.
EOT
	if test ${totalm} -gt 0 ; then
		grader_msg_ << EOT 
${descr}: ${goodm} out of ${totalm} correct match results.
EOT
	fi
}

grader_banner 1 0
show_io_tallies "${basiciotallydir}/missingdata" "Missing NFA definition file"
show_io_tallies "${basiciotallydir}/emptydata" "Empty NFA definition file"
